...
def parallel_plot(x1: np.ndarray, y1: np.ndarray, x2: np.ndarray, y2: np.ndarray,
    ...
    """
    if x1.shape != y1.shape or min(x1.shape) == 1 or x2.shape != y2.shape or min(x2.shape) == 1 or min(x2.shape) == 4: #test nie działał gdy wektry x1,x2,y1,y2 nie były tego samego wymiaru, a powinna być taka możliwość
        return None
...
def log_plot(x: np.ndarray, y: np.ndarray, xlabel: str, ylabel: str, title: str, log_axis: str): # xlabel było ustawione jako ndarray a powinnien być str więc to zmieniłem
